...
const { username, pswd, server, dbName, restaurants_DB } = require('../constants');
...
const resolvers = {
    ...
    Mutation: {
      ...
      changeRestaurantData: (parent,args) => changeRestaurantData(parent,args)
    }
};
...
async function addPerson(parent,args){
  ...
  mongoose.connect(person_url,{ useNewUrlParser: true, useUnifiedTopology: true });
...
async function getRestaurants(){  
  await mongoose.connect(rest_url,{ useNewUrlParser: true, useUnifiedTopology: true });
  mongoose.connection.once('open',() => {
  ...
  const data = await mongoose.connection.db.collection('restaurants')
    .find({cuisine :  "Indian"}).limit(5).toArray();  
  mongoose.connection.close(() => console.log('connection closed'))      
  return data;
}
...
async function changeRestaurantData(parent,args){  
  ...
  const data = (await mongoose.connection.db.collection('restaurants')
    .find({restaurant_id : args.restaurant_id}).toArray())[0];
  ...
  /* Update the db */
  await mongoose.connection.db.collection('restaurants').findOneAndUpdate(
    {restaurant_id : args.restaurant_id}, { $set: data } , {upsert: true});
